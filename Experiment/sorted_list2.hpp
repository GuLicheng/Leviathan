...
namespace leviathan::collections
{
    ...
    class sorted_list
    {
        ...
        struct sorted_list_iterator
        {
            ...
            {
                const auto idx = m_c->buckets_ref()[m_out_idx][m_in_idx]; 
                return m_c->m_impl.m_slot[idx];
            }
    ...
    private:
        ...
        using index_alloc_type = alloc_traits::template rebind_alloc<index_type>;
        
        // vector<size_t>
        using bucket_type = buffer<index_type>;
        using bucket_alloc_type = alloc_traits::template rebind_alloc<bucket_type>;

        // vector<vector<size_t>>
        using bucket_list_type = buffer<bucket_type>;
        using bucket_list_alloc_type = alloc_traits::template rebind_alloc<bucket_list_type>;
        
        // vector<T>
        using slot_type = buffer<value_type>;
        using slot_alloc_type = alloc_traits::template rebind_alloc<slot_type>;
        using slot_alloc_traits = alloc_traits::template rebind_traits<slot_type>;
        ...
        struct sorted_list_impl
        {
            ...
            size_t m_size = 0;          
            bucket_list_type m_buckets;             // Save indices
            slot_type m_slot;                       // Save data
    ...
    // public:
        ...
        void clear_buckets_and_slot()
        {
            // Call dtor for outer buffer.
            std::ranges::for_each(buckets_ref(), [this](auto& bucket) { bucket.dispose(m_alloc); });
            buckets_ref().dispose(m_alloc);
            slot_ref().dispose(m_alloc);
        }
		...
		iterator remove_impl(const_iterator cpos) 
		{
			...
			{
				buckets_ref().erase(m_alloc, buckets_ref().begin() + i);
				return { this, i, j };
			}
			shrink(i);	
		...
		void shrink(size_t pos)
		{
            ...
            if (buckets_ref()[pos].size() < TruckSize / 2 && buckets_size() > 1)
            {
                ...
                {
                    buckets_ref().erase(m_alloc, buckets_ref().begin() + pos);
                }
                else
                {
        ...
        bool check_equal(const K& x, iterator pos)
        {
            ...
            return !compare_ref()(x, KeyOfValue()(*pos));
        }
		...
		std::pair<std::size_t, std::size_t> find_item_by_key(const K& k) const 
		{
            auto bucket = std::ranges::lower_bound(buckets_ref(), k, compare_ref(), 
                [this](const auto& b) -> auto& { 
                    ...
                    const auto idx = b.back();
                    return KeyOfValue()(slot_ref()[idx]); 
                }
            );
            ...
            auto iter = std::ranges::lower_bound(*bucket, k, compare_ref(),
                [this](const auto& s) -> auto& {
                    // s is slot.
                    return KeyOfValue()(slot_ref()[s]);
                }
            );
        ...
        */
        bool try_expand(size_t pos)
        {
            ...
            if (bucket.size() > TruckSize * 2)
            {
                ...
                buckets_ref().emplace(m_alloc, buckets_ref().begin() + pos + 1, half);
                return true;
            }
            return false;
        }
        ...
        std::pair<iterator, bool> insert_impl(U&& val)
        {
			...
			if (buckets_size())
			{
				...
				if (i == buckets_size())
				{
                    ...
                    slot_ref().emplace_back(m_alloc, (U&&) val);
					buckets_ref().back().emplace_back(m_alloc, slot_ref().size() - 1);
					i = buckets_size() - 1;
				...
				else
				{
                    ...
                    const auto idx = buckets_ref()[i][j];
					if (!compare_ref()(KeyOfValue()(val), KeyOfValue()(slot_ref()[idx])))
					{
						succeed = false;
					}
					else
					{
                        // Construct value at end.
                        slot_ref().emplace_back(m_alloc, (U&&) val);
						auto& v = buckets_ref()[i];
						auto ret_iter = v.emplace(m_alloc, v.begin() + j, slot_ref().size() - 1);
			...
			else
			{
                slot_ref().emplace_back(m_alloc, (U&&) val);
                bucket_type b;
                b.emplace_back(m_alloc, 0);
				buckets_ref().emplace_back(m_alloc, b);
				out_idx = in_idx = 0;
				succeed = true;
			}
        ...
        auto& value_ref(size_t i, size_t j)
        { 
            const auto idx = buckets_ref()[i][j];
            return slot_ref()[idx]; 
        }

        auto& value_ref(size_t i, size_t j) const
        { 
            const auto idx = buckets_ref()[i][j];
            return slot_ref()[idx]; 
        }
    ...
    class sorted_map : public sorted_list<std::tuple<K, V>, Compare, Allocator, select1st, true> { };
