/*
    The old version use buffer(something like std::vector) as inner table. However,
    STL map containers store std::pair<const K, V> as their inner implementation.
    For buffer, if the value type is std::pair<const K, V>, the insert may call
    std::move/std::rotate, these algorithms is invalid for std::pair<const K, V>.
    So we just use buffer to save indices which are integers and the value will be 
    stored in another buffer. In the case, we can use insert method for indices.
    Unfortunately, the will make insert/search/remove much more slower(from faster
    then std::set to lower than std::set).
*/
...
namespace leviathan::collections
{
    ...
    private:
        ...
        using bucket_type = buffer<value_type>;
