...
public:
    ...
    struct info
    {
        ...
        named_tuple<
            ...
            field<"nargs", std::string_view, [] { return 1; }>,
            field<"const", std::string_view>,
            field<"require", std::string_view>> m_other;
    ...
    void add_argument(Args... args)
    {
        ...
        auto register_name = [this, &i]<typename T>(T arg)
        {
            // check
            if constexpr (is_tag_value<T>::value)
            {
                i.m_other[arg.tag()] = arg.value;
            }
            else if constexpr (std::is_convertible_v<T, std::string_view>)
            {
                std::string_view sv { arg };
                i.m_arg_names.emplace_back(sv);
                auto name = leviathan::drop_while(sv, [](char ch) { return ch == '-'; });
                if (this->m_names.count(name))
                    this->m_errlog.emplace_back(cat_string("Name ", name, " Already Existed"));
                else this->m_names.emplace(name);
            }
            else
                LV_STATIC_ASSERT_FALSE("Unknown");
        };

        // (register_name(std::move(args)), ...);


        // check const
        if (i.m_other["const"_arg] && i.m_other["default"_arg].empty())
    ...
    std::pair<parser_result, bool> parse_args(int argc, char const *argv[]) 
    {
        ...
        for (int i = 1; i < argc; ++i)
        {
            ...
            else
            {
                ...
                m_args[indices[idx]].m_other["default"] = arg;
            }
        ...
        for (auto& info : m_args)
        {
            ...
            // if (info.m_other["require"] && info.m_other["default"].empty())
            //     this->m_errlog.emplace_back(cat_string("Required Argument ", info.m_arg_names[0], " is Null"));
            // ret.m_values.emplace_back(info.m_other["default"]);
...
int main(int argc, char const *argv[])
{
    ...
    parser.add_argument("--version", "-v", arg<"const"> = true, arg<"default"> = "0.0.0");
    parser.add_argument("--epoch", arg<"default"> = "15", arg<"help"> = "epoch num of your training");
    parser.add_argument("--lr", arg<"default"> = "15");
    parser.add_argument("nooptional", true);
