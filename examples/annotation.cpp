...
int main(int argc, char const *argv[])
{
    ...
    assert(result.std_string_val == "Hello, World!");
    assert(result.c_string_ptr == nullptr);
    assert(result.vector_val == std::vector<int>{1, 2, 3});
    assert(result.map_val.empty());
    assert(result.map_val.size() == 0);
