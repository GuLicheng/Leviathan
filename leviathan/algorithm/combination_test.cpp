...
TEST_CASE("combination random access iterator")
{
    ...
    } while (cpp::algorithm::next_combination(vec.begin(), vec.begin() + 2, vec.end()).found);
}
...
TEST_CASE("range_combination")
{
    ...
    do {
        REQUIRE(vec == result[i++]);
    } while (cpp::algorithm::next_combination(vec, vec.begin() + 2).found);
}
...
TEST_CASE("combination bidirectional iterator")
{
    ...
    do {
        REQUIRE(vec == result[i++]);
    } while (cpp::algorithm::next_combination(vec.begin(), std::next(vec.begin(), 2), vec.end()).found);
}
