...
consteval std::string_view name_of()
{
...
#if defined(_MSC_VER)
    // __FUNCSIG__
    const auto first = name.find("name_of") + sizeof("name_of");
    const auto last = name.rfind("void");
    return name.substr(first, last - first - 2);

#elif defined(__clang__)
    // __PRETTY_FUNCTION__
    // consteval std::string_view name_of() [T = int]
    ...
    // __PRETTY_FUNCTION__
    // consteval std::string_view name_of() [with T = std::__cxx11::basic_string<char>; std::string_view = std::basic_string_view<char>]
    const auto first = name.find_first_of('=') + 2;
    const auto last = name.find_last_of(';');
    return name.substr(first, last - first);
