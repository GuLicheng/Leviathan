...
public:
    ...
    void add_value(I first, I last) 
    {
        m_values.emplace_back(first, last);
    }

private:
    std::vector<std::string_view> m_ops;
    std::vector<std::pair<I, I>> m_values;
...
struct nargs
{
    ...
    constexpr static I operator()(I first, S last, parse_context<I>& ctx)
    {
        auto second = std::ranges::next(first, N, last);
        ctx.add_value(first, second);
        return second;
    }
};
...
struct variadic
{
    ...
    {
        return std::ranges::find_if(first, last, [](const char* name) {
            // For empty string, the name[0] must be '0', so
            // deference name[0] will not cause error.
            return name[0] == '-'; 
        });
    }
};
