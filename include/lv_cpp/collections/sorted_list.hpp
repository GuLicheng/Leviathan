...
namespace leviathan::collections
{
	...
	*/
	...
	public:
		...
		sorted_list_impl() noexcept(std::is_nothrow_default_constructible_v<key_compare>) 
			: m_lists{ }, m_cmp{ }, m_lists{ } { }

		sorted_list_impl(Compare cmp) : m_lists{ }, m_cmp { std::move(cmp) }, m_size { 0 } { }

		sorted_list_impl(const sorted_list_impl&) = default;
		sorted_list_impl(sorted_list_impl&&) 
			noexcept(std::is_nothrow_move_constructible_v<Compare> && std::is_nothrow_move_constructible_v<outer_container>) = default;

		sorted_list_impl& operator=(const sorted_list_impl&) = default;
		sorted_list_impl& operator=(sorted_list_impl&&) 
			noexcept(std::is_nothrow_move_constructible_v<self_type>) = default;
	...
	private:
		...
		[[no_unique_address]] key_compare m_cmp;
		size_type m_size;
	...
	using sorted_list = sorted_list_impl<T, Compare, false, set_config<T, Compare, std::allocator<T>>>;
