...
// Based on stl_tree.h from libstdc++.
// Changes by Daniel K. O. <danielosmari@users.sourceforge.net> - 2005
...
namespace avl
{
  /* AVL tree class, designed for use in implementing STL
     ...
     insertion and deletion algorithms were written based on:
        * Donald Knuth. "The Art of Computer Programming": Searching and Sorting Algorithms.
        * a lot of wasted pencil and papers while debugging the deletion algorithm.
   */
  ...
  template<typename _Tp>
    struct _avl_tree_iterator
    {
      ...
      _avl_tree_iterator(_Link_type __x)
      : _M_node(__x) { }
  ...
  template<typename _Tp>
    struct _avl_tree_const_iterator
    {
      ...
      _avl_tree_const_iterator(_Link_type __x)
      : _M_node(__x) { }
  ...
  template<typename _Key, typename _Val, typename _KeyOfValue,
    ...
    class _avl_tree
    {
      typedef typename _Alloc::template rebind<_avl_tree_node<_Val> >::other
              _Node_allocator;
	...
	__tmp->_M_bal_factor = __x->_M_bal_factor;
	__tmp->_M_left = 0;
	__tmp->_M_right = 0;
	return __tmp;
      }
    ...
    protected:
	  ...
	  _avl_tree_impl(const _Node_allocator& __a = _Node_allocator(),
			const _Key_compare& __comp = _Key_compare())
	  : _Node_allocator(__a), _M_key_compare(__comp), _M_node_count(0)
	  {
	    //this->_M_header._M_bal_factor = 0;
	    this->_M_header._M_bal_factor = -2;        // need this for _avl_tree_decrement
      ...
      template<typename _Key_compare>
        struct _avl_tree_impl<_Key_compare, true> : public _Node_allocator
	{
	  _Key_compare 		_M_key_compare;
	  _avl_tree_node_base 	_M_header;
	  size_type 		_M_node_count; // Keeps track of size of tree.
    ...
    public:
      ...
      _avl_tree(const _Compare& __comp, const allocator_type& __a)
      : _M_impl(__a, __comp)
      { }

      _avl_tree(const _avl_tree<_Key,_Val,_KeyOfValue,_Compare,_Alloc>& __x)
	...
	if (__x._M_root() != 0)
      ...
      void
      swap(_avl_tree<_Key,_Val,_KeyOfValue,_Compare,_Alloc>& __t);
      ...
      // Debugging.
      /* TODO (DanielKO#1#): Write a verification routine for AVL. */
/*      bool
      __rb_verify() const;*/
    };
  ...
  template<typename _Key, typename _Val, typename _KeyOfValue,
           typename _Compare, typename _Alloc>
    inline void
    _avl_tree<_Key,_Val,_KeyOfValue,_Compare,_Alloc>::erase(iterator __position)
    {
      _Link_type __y =
	static_cast<_Link_type>(_avl_tree_rebalance_for_erase(__position._M_node,
							     this->_M_impl._M_header));
      destroy_node(__y);
      --_M_impl._M_node_count;
    }
  ...
  template<typename _Key, typename _Val, typename _KeyOfValue,
    ...
    _avl_tree<_Key,_Val,_KeyOfValue,_Compare,_Alloc>::erase(const _Key& __x)
    {
  ...
  template<typename _Key, typename _Val, typename _KeyOfValue,
           typename _Compare, typename _Alloc>
    void
    _avl_tree<_Key,_Val,_KeyOfValue,_Compare,_Alloc>::_M_erase(_Link_type __x)
    {
  ...
  template<typename _Key, typename _Val, typename _KeyOfValue,
           typename _Compare, typename _Alloc>
    typename _avl_tree<_Key,_Val,_KeyOfValue,_Compare,_Alloc>::iterator
    _avl_tree<_Key,_Val,_KeyOfValue,_Compare,_Alloc>::find(const _Key& __k)
    {
  ...
  template<typename _Key, typename _Val, typename _KeyOfValue,
           typename _Compare, typename _Alloc>
    inline
    std::pair<typename _avl_tree<_Key,_Val,_KeyOfValue,
  ...
  template<typename _Key, typename _Val, typename _KoV,
           typename _Compare, typename _Alloc>
    inline
    std::pair<typename _avl_tree<_Key, _Val, _KoV,
			   _Compare, _Alloc>::const_iterator,
	 typename _avl_tree<_Key, _Val, _KoV, _Compare, _Alloc>::const_iterator>
    _avl_tree<_Key, _Val, _KoV, _Compare, _Alloc>::
    equal_range(const _Key& __k) const
    { return std::pair<const_iterator, const_iterator>(lower_bound(__k),
						  upper_bound(__k)); }
/*      Not used by the AVL tree.
  unsigned int
  _avl_tree_black_count(const _avl_tree_node_base* __node,
                       const _avl_tree_node_base* __root);
*/
/*
        Not used by the AVL tree.
        TODO: implement an __avl_verify() routine.

  template<typename _Key, typename _Val, typename _KeyOfValue,
           typename _Compare, typename _Alloc>
    bool
    _avl_tree<_Key,_Val,_KeyOfValue,_Compare,_Alloc>::__rb_verify() const
    {
